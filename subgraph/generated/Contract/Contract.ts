...
export class Contract__verifyResult {
  value0: boolean;
  value1: Address;
...
export class Contract__verifyInput_reqStruct extends ethereum.Tuple {
  get to(): Address {
    return this[0].toAddress();
  }
...
export class Contract extends ethereum.SmartContract {
  ...
  verify(
    _req: Contract__verifyInput_reqStruct,
    _signature: Bytes
  ): Contract__verifyResult {
    let result = super.call(
      "verify",
    ...
    return new Contract__verifyResult(
      result[0].toBoolean(),
      result[1].toAddress()
    );
  }
  ...
  try_verify(
    _req: Contract__verifyInput_reqStruct,
    _signature: Bytes
  ): ethereum.CallResult<Contract__verifyResult> {
    let result = super.tryCall(
      "verify",
      "verify((address,address,uint256,uint256,address,uint128,uint128,bytes32),bytes):(bool,address)",
      [ethereum.Value.fromTuple(_req), ethereum.Value.fromBytes(_signature)]
    ...
    let value = result.value;
    return ethereum.CallResult.fromValue(
      new Contract__verifyResult(value[0].toBoolean(), value[1].toAddress())
    );
  }
}
